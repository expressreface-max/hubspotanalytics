...
import { searchAllContacts, fetchZipTerritoryMap, HubSpotError } from "@/lib/hubspot"
...
export async function POST(req: Request) {
  ...
  // HubSpot contact search: created on/after the window start AND zip IN the list.
  const filterGroups = [
    {
      filters: [
  ...
  try {
    const [contacts, zipTerritory] = await Promise.all([
      searchAllContacts(token, filterGroups, ["createdate", "zip"]),
      fetchZipTerritoryMap(token, zips),
    ])
    ...
    for (const c of contacts) {
      const z = normalizeZip(c.properties.zip || "")
      const row = counts.get(z)
      if (!row) continue
      const created = c.properties.createdate
      if (!created) continue
      const key = monthKeyOf(new Date(created).getTime())
    ...
    const rows = zips.map((zip) => {
      ...
      return { zip, territory: zipTerritory[zip] || "", counts: byMonth, total }
    })
    ...
    return NextResponse.json({ error: (err as Error).message || "Failed to load contacts" }, { status: 500 })
  }
}
