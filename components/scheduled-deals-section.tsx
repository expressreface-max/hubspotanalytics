...
import { RefreshCw, AlertTriangle, Download, CalendarClock } from "lucide-react"
...
// Deals that entered the "Scheduled" stage within the Funnel page's selected
// date range (passed down as dateFrom/dateTo, plus the optional pipeline
// filter), dated by the stage-entered date. Same layout as ClosedWonSection.
// Rendered at the bottom of the Funnel page.
export function ScheduledDealsSection({
  configured,
  dateFrom,
  dateTo,
  pipelines,
  ...
  const report = useQuery({
    queryKey: ["scheduled-deals", dateFrom, dateTo, pipelines.join(",")],
    queryFn: () => apiPost<Report>("/api/hs/scheduled-deals", { dateFrom, dateTo, pipelines }),
    enabled: configured,
  })
  ...
  function exportCsv() {
    const header = ["Deal", "Scheduled date", "Amount", "Sales rep", "Pipeline"]
    const body = deals.map((d) => [d.name, fmtDate(d.scheduledDate), d.amount, d.ownerName, d.pipeline])
    ...
    const download = () => {
      ...
      a.href = url
      a.download = `scheduled-deals-${dateFrom.slice(0, 10)}_${dateTo.slice(0, 10)}.csv`
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col gap-3 pb-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <CardTitle className="text-base">Scheduled deals</CardTitle>
          <p className="mt-1 text-sm text-muted-foreground">
            Deals that entered the Scheduled stage during {periodText}
            {loading ? null : (
              <>
                {" "}
      ...
      <CardContent>
        {report.isError || report.data?.error ? (
          ...
          <p className="flex items-center justify-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            No deals entered the Scheduled stage in the selected period.
          </p>
        ) : (
            ...
            <table className="w-full min-w-[640px] text-sm">
              <thead className="sticky top-0 z-10 bg-card">
                <tr className="border-b text-xs uppercase tracking-wide text-muted-foreground">
                  ...
                  <th className="px-2 py-2 text-right font-medium">Scheduled date</th>
